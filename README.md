...
Stack: AudioSalad API · Anthropic API (Claude) · Python

**-Release Calendar Automation — Client: 6149 (music distributor)**

6149's sync and promo teams work off a release calendar — a forward-looking view of what's coming out and when — to pitch tracks for sync placements, line up playlist and PR pushes, and keep releases from colliding. Right now that calendar is a Google spreadsheet an admin updates by hand every few days, manually pulling each upcoming release from the distribution backend. It's tedious, easy to let drift, and only as current as the last time someone remembered to update it. This automates that step entirely: it pulls upcoming releases straight from the AudioSalad API and writes them into the spreadsheet automatically, so the team keeps the exact tool and workflow they already use — just always up to date, with no manual entry.

Stack: Apps Script · AudioSalad API · Google Sheets
